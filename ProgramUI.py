...
class SimpleTextBoxForm(Form):
    def __init__(self):
        ...
        self.mappingRowNumber = []
        self.mappingRowStructure = []
    ...
    def btnNextPage_Clicked(self, sender, evtArgs):
        print("btnNextPage_Clicked")
        if(self.recvTextBoxCheck()):
            print("Next Page")


    def recvTextBoxCheck(self):
        isOK = True
        for panel in self.ListPanel.Controls:
            for control in panel.Controls :
                if (control.GetType() == type(TextBox())):
                    print("Text is \"" + control.Text + "\"")
                    if control.Text == "":
                        isOK = isOK & False
                        break
        
        print("Return " + str(isOK))
        return isOK
